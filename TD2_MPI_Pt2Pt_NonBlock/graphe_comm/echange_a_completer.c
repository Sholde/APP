        ...
        char **msg_rcv, int *taille_msg_rcv) 
{
  ...
  for (int i = 0; i < graphe->nb_noeuds; i++)
    {
      int nb_voisins = graphe->nb_voisins[i];
      MPI_Request req[nb_voisins];
      
      for (int j = 0; j < nb_voisins; j++)
        {
          MPI_Isend(msg_snd + count * i + j, taille_msg_snd[count * i + j], MPI_CHAR, graphe->voisins[i][j], MPI_COMM_WORLD, req + (int *)j);
          MPI_Irecv(msg_rcv + count * i + j, taille_msg_rcv[count * i + j], MPI_CHAR, graphe->voisins[i][j], MPI_COMM_WORLD, req + (int *)j);
          count++;
        }

      MPI_Waitall(nb_voisins, req, MPI_STATUS_IGNORE);
    }
}
